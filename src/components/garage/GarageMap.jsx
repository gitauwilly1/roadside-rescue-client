...
const GarageMap = ({ clientLocation, garageLocation, onRouteCalculated }) => {
  ...
  useEffect(() => {
    ...
    const center = garageLocation 
      ? [garageLocation.latitude || garageLocation.coordinates[1], garageLocation.longitude || garageLocation.coordinates[0]]
      : [-1.2921, 36.8219];
  ...
  // Update garage marker (current location)
  useEffect(() => {
  ...
  // Update client marker and calculate route
  useEffect(() => {
    if (!mapInstanceRef.current || !mapLoaded) return;
    ...
    if (clientLocation) {
      ...
      if (clientMarkerRef.current) {
        ...
        clientMarkerRef.current = L.marker(position, { icon: clientIcon })
          .addTo(mapInstanceRef.current)
          .bindPopup('<strong>📍 Client Location</strong><br/>Emergency rescue location');
      }
      ...
      {(distance || duration) && (
        <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm px-4 py-2 rounded-lg shadow-md z-10">
          <div className="space-y-1">
            <p className="text-xs text-gray-500 font-medium">NAVIGATION INFO</p>
